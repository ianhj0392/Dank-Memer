...
module.exports = new GenericCurrencyCommand(
  async ({ Memer, msg, addCD, isGlobalPremiumGuild, userEntry, donor }) => {
    let user = msg.author;
    let total = await Memer.calcMultiplier(Memer, user, userEntry, donor, msg, isGlobalPremiumGuild);
    let show = await Memer.showMultiplier(Memer, user, userEntry, donor, msg, isGlobalPremiumGuild);
    await addCD();
    return {
